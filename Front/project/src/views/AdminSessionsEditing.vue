                ...
                <v-card>
                    ...
                    <v-card-text>
                        ...
                        <v-select
                            ...
                            v-model="modelHall"
                        >
                            <template v-slot:item="props">{{props.item.number}}</template>
                            <template v-slot:selection="props">{{props.item.number}}</template>
                        </v-select>
...
export default {
    ...
    methods: {
        search() {
            if (this.searchId)
                this.$store.dispatch("Sessions/GET_SESSION_LIST", {
                    id: this.searchId
                });
        },
        confirmDialog() {
            if (
                !this.editingItem.start ||
                this.editingItem.start.length < 4 ||
                !this.editingItem.end ||
                this.editingItem.end.length < 4 ||
                !this.editingItem.idHall ||
                this.editingItem.idHall.length == 0 ||
                !this.editingItem.idFilm ||
                this.editingItem.idFilm.length == 0
            )
                return;
            setTimeout(() => {
                if (this.editingIndex == -1) {
                    this.$store
                        .dispatch("Sessions/ADD_SESSION", this.editingItem)
                        .then(() => this.search());
                } else {
                    this.$store
                        .dispatch("Sessions/CHANGE_SESSION", this.editingItem)
                        .then(() => this.search());
                }
                this.dialog = false;
            }, 100);
        ...
        changeItem(item, index) {
            ...
            this.editingItem.start = item.start.substring(
                item.start.length - 5
            );
        ...
        deleteItem(item) {
            this.$store
                .dispatch("Sessions/DELETE_SESSION", { id: item.id })
                .then(() => this.search());
        }
    },
    computed: {
